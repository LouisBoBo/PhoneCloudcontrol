...
//            mRotateView.setRotation((1-mDragOffset)*180);
            ...
            directionView = findViewById(R.id.img_Direction);

            if(directionView !=null){
                if(mDragOffset > 0.5f){//下降
                    directionView.animate().rotation(360);
                    Log.d("tag1","下降");
                }
                else {//上升
                    directionView.animate().rotation(180);
                    Log.d("tag1","上升");
                }
            }
            requestLayout();
