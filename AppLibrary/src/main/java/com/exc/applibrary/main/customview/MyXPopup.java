...
public class MyXPopup {
    ...
    public static class Builder {
         ...
         * @return
         */
        public MyLoadingPopupView asLoading(CharSequence title, int bindLayoutId) {
            popupType(PopupType.Center);
            MyLoadingPopupView popupView = new MyLoadingPopupView(this.context, bindLayoutId)
                    .setTitle(title);
            popupView.popupInfo = this.popupInfo;
            return popupView;
        }

        public MyLoadingPopupView asLoading(CharSequence title) {
            return asLoading(title, 0);
        }

        public MyLoadingPopupView asLoading() {
